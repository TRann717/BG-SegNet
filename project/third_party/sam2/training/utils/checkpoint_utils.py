...
def with_check_parameter_frozen(
    ...
    """
    Context manager that inspects a models surrounding a piece of code
    and verifies if the models has been updated by this piece of code

    The function will raise an exception if the models has been updated
    on at least one of the parameter that matches one of the pattern

    Args:
        model: the models that might have been updated
    ...
    if not np.allclose(summary_before, summary_after, atol=1e-6):
        raise ValueError(
            ...
            You can resolve this error by either initializing those parameters from within the models definition
...
class CkptExcludeKernel:
    """
    Removes the keys from the given models state_dict that match the key_pattern.
...
def get_state_dict(checkpoint, ckpt_state_dict_keys):
    if isinstance(checkpoint, RecursiveScriptModule):
        # This is a torchscript JIT models
...
def load_checkpoint_and_apply_kernels(
    ...
    Args:
        ...
        checkpoint_kernels List(Callable): A list of checkpoint processing kernels
            ...
            given order.
        ckpt_state_dict_keys (str): Keys containing the models state dict.
        map_location (str): a function, torch.device, string or a dict specifying how to
            remap storage locations
...
def load_state_dict_into_model(
    ...
    """
    Loads a state dict into the given models.

    Args:
        state_dict: A dictionary containing the models's
