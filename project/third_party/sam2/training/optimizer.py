...
def validate_param_group_params(param_groups: List[Dict], model: nn.Module):
    ...
    Args:
        ...
        model: Model to validate against. The check ensures that all the models
    ...
    assert set.union(*parameters) == model_parameters, (
        "Scheduler generated param_groups must include all parameters of the models."
        f" Found {len(set.union(*parameters))} params whereas models has"
    ...
    for module_cls_name in filter_module_cls_names:
        ...
        if module_cls not in module_cls_to_param_names:
            raise AssertionError(
                ...
                "match any classes in the models"
            )
        matching_parameters = module_cls_to_param_names[module_cls]
        assert (
            len(matching_parameters) > 0
        ), f"module_cls_name {module_cls_name} does not contain any parameters in the models"
        logging.info(
...
def unix_param_pattern_to_parameter_names(
    ...
    for param_name in filter_param_names:
        ...
        ), f"param_name {param_name} does not match any parameters in the models"
...
def construct_optimizer(
    ...
    Args:
        model: models to perform stochastic gradient descent
        ...
        validate_param_groups: If enabled, valides that the produced param_groups don't
            overlap and cover all the models parameters.
    """
...
def layer_decay_param_modifier(
    ...
    - models: a models that implements a method `get_layer_id` that maps layer_name to an integer and
            and a method get_num_layers.
            Alternatively, use apply_to argument to select a specific component of the models.
    - layer_decay_value: float
    - layer_decay_min: min val for layer decay
    - apply_to: optional arg to select which component of the models to apply the the layer decay modifier to
