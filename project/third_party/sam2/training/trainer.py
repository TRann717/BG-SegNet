...
class Trainer:
    ...
    def _init_model_state(self):
        ...
        # We do this check here before even saving the models to catch errors
        ...
        # within the models definition, unless `initialize_after_preemption`
    ...
    def _run_step(
        ...
        # grads will also update a models even if the step doesn't produce
        # gradients
...
def print_model_summary(model: torch.nn.Module, log_dir: str = ""):
    """
    Prints the models and the number of parameters in the models.
    ...
    logging.info("==" * 10)
    logging.info(f"Summary for models {type(model)}")
    ...
    if log_dir:
        output_fpath = os.path.join(log_dir, "models.txt")
