"""
Semantic segmentation loss for PH2 dermoscopic lesions.

Integrates region losses (CE + Dice/Tversky) with unified soft boundary supervision
and optional geometric band regularizers, fully aligned with current model outputs
and configuration.
"""
...
class CompoundLoss(nn.Module):
    """
    Composite loss for dermoscopic lesion segmentation.
    ...
    def forward(self, outputs: dict, targets: torch.Tensor):
        ...
        """
        # Numerical stability check for logits.
        ...
        if torch.isnan(l_ce) or torch.isinf(l_ce):
            print(f"[WARN] CE loss is NaN/Inf, replacing with 0.0")
            l_ce = torch.tensor(0.0, device=logits.device, dtype=logits.dtype)
        total += l_ce
        loss_dict['ce'] = l_ce.detach()

        if self.dice_w > 0:
            l_overlap = self.overlap(logits, targets) * self.dice_w
            if torch.isnan(l_overlap) or torch.isinf(l_overlap):
                print(f"[WARN] Dice loss is NaN/Inf, replacing with 0.0")
                l_overlap = torch.tensor(0.0, device=logits.device, dtype=logits.dtype)
            total += l_overlap
        ...
        if ('aux' in outputs) and (self.aux_w > 0):
            ...
            if torch.isnan(aux).any() or torch.isinf(aux).any():
                aux = torch.clamp(aux, min=-50.0, max=50.0)
            ...
            if torch.isnan(l_aux) or torch.isinf(l_aux):
                print(f"[WARN] Aux CE loss is NaN/Inf, replacing with 0.0")
        ...
        if self.bd_enable and ('boundary' in outputs):
            if self.bd_class_aware and 'boundary_b4' in outputs:
                ...
                for c in range(1, self.num_classes):  # skip background class 0
                    ...
                    if torch.isnan(l_bd_c) or torch.isinf(l_bd_c):
                        print(f"[WARN] Boundary loss for class {c} is NaN/Inf, skipping this class")
                        continue
                ...
                if valid_classes > 0:
                    ...
                    print(f"[WARN] No valid classes for boundary loss, setting to 0.0")
                    bd_loss_total = torch.tensor(0.0, device=logits.device, dtype=logits.dtype)
                
                w_bd = self.bd_weight_max * self._bd_scalar
                bd_contribution = w_bd * bd_loss_total
                if torch.isnan(bd_contribution) or torch.isinf(bd_contribution):
                    print(f"[WARN] Boundary contribution is NaN/Inf, skipping")
                    bd_contribution = torch.tensor(0.0, device=logits.device, dtype=logits.dtype)
                total = total + bd_contribution
                ...
                if pred_1x is not None:
                    ...
                    if torch.isnan(l_bd) or torch.isinf(l_bd):
                        print(f"[WARN] Unified boundary loss is NaN/Inf, replacing with 0.0")
                        l_bd = torch.tensor(0.0, device=logits.device, dtype=logits.dtype)
                    bd_contribution = w_bd * l_bd
                    if torch.isnan(bd_contribution) or torch.isinf(bd_contribution):
                        bd_contribution = torch.tensor(0.0, device=logits.device, dtype=logits.dtype)
                    total = total + bd_contribution
        ...
        if 'dir_b4' in outputs and self.config['model']['boundary'].get('dir_weight', 0) > 0:
            ...
            if torch.isnan(dir_loss) or torch.isinf(dir_loss):
                print(f"[WARN] Direction loss is NaN/Inf, replacing with 0.0")
                dir_loss = torch.tensor(0.0, device=logits.device, dtype=logits.dtype)
        ...
        if self.config['model']['boundary'].get('consis_weight', 0) > 0 and 'boundary_b4' in outputs and 'boundary_b8' in outputs:
            ...
            if torch.isnan(consis_loss) or torch.isinf(consis_loss):
                print(f"[WARN] Consistency loss is NaN/Inf, replacing with 0.0")
                consis_loss = torch.tensor(0.0, device=logits.device, dtype=logits.dtype)
        ...
        if self.config['model']['boundary'].get('margin_weight', 0) > 0 and 'feat_1_4' in outputs:
            ...
            if isinstance(margin_loss, torch.Tensor):
                if torch.isnan(margin_loss) or torch.isinf(margin_loss):
                    print(f"[WARN] Margin loss is NaN/Inf, replacing with 0.0")
                    margin_loss = torch.tensor(0.0, device=logits.device, dtype=logits.dtype)
            elif not isinstance(margin_loss, (int, float)) or (isinstance(margin_loss, float) and (np.isnan(margin_loss) or np.isinf(margin_loss))):
                margin_loss = 0.0
        ...
        if self.band_supcon is not None and 'feat_1_4' in outputs:
            try:
                ...
                if torch.isnan(l_bctr) or torch.isinf(l_bctr):
                    print(f"[WARN] Band supcon loss is NaN/Inf, replacing with 0.0")
                    l_bctr = torch.tensor(0.0, device=logits.device, dtype=logits.dtype)
                total = total + self.band_supcon_w * l_bctr
                loss_dict['band_supcon'] = l_bctr.detach()
            except Exception as e:
                print(f"[WARN] Band supcon loss computation failed: {e}, skipping")
        ...
        if self.curv_aware_enable and self.curv_aware is not None:
            try:
                ...
                if torch.isnan(l_curv) or torch.isinf(l_curv):
                    print(f"[WARN] Curvature aware loss is NaN/Inf, replacing with 0.0")
                    l_curv = torch.tensor(0.0, device=logits.device, dtype=logits.dtype)
                total = total + l_curv  # Weight already embedded in CurvAwareLoss.lam
                loss_dict['curv_aware'] = l_curv.detach()
            except Exception as e:
                print(f"[WARN] Curvature aware loss computation failed: {e}, skipping")
                loss_dict['curv_aware'] = torch.tensor(0.0, device=total.device)

        if torch.isnan(total) or torch.isinf(total):
            print(f"[ERROR] Total loss is NaN/Inf after all protections! loss_dict: {loss_dict}")
            print(f"        This usually indicates a serious numerical issue. Returning a small safe value.")
            total = torch.tensor(0.1, device=logits.device, dtype=logits.dtype, requires_grad=True)
