"""
Trainer for PH2-style dermoscopic lesion segmentation with boundary-aware losses and open-set handling.
"""
import os
import time
import torch
...
class Trainer:
    """PH2 trainer for medical image segmentation with curriculum and boundary gating."""
    ...
    def __init__(self, model, config, device='cuda', save_dir: Optional[Path] = None, logger=None):
        ...
        # Segmentation loss (compound region + boundary terms)
        self.criterion = CompoundLoss(config)
        
        # Automatic mixed precision (AMP)
        if self.amp:
            amp_cfg = config.get("train", {})
            init_scale = float(amp_cfg.get("amp_init_scale", 1024.0))
            growth_factor = float(amp_cfg.get("amp_growth_factor", 2.0))
            backoff_factor = float(amp_cfg.get("amp_backoff_factor", 0.5))
            growth_interval = int(amp_cfg.get("amp_growth_interval", 2000))
        ...
        # Region-based metrics on-the-fly
    ...
    def _build_optimizer(self):
        """Build optimizer with differential learning rates for boundary modules."""
        base_lr = self.config['train']['lr']
        wd = self.config['train']['weight_decay']
        
        # Parameter grouping: boundary modules use a lower learning rate.
        boundary_params = []
        main_params = []
    ...
    def _adjust_learning_rate_warmup(self):
        """Adjust learning rate with linear warmup for all parameter groups."""
    ...
    def train_epoch(self, train_loader, epoch):
        ...
        # Optionally unfreeze boundary gate gradients at a predefined epoch.
        ...
        for i, batch in enumerate(pbar):
            ...
            # Basic NaN/Inf checks on input tensors for numerical stability.
            if torch.isnan(images).any() or torch.isinf(images).any():
                print(f"[ERROR] Batch {i}: Input images contain NaN/Inf! nan_count={torch.isnan(images).sum()}, inf_count={torch.isinf(images).sum()}")
                continue
            if torch.isnan(masks).any() or torch.isinf(masks.float()).any():
                print(f"[ERROR] Batch {i}: Input masks contain NaN/Inf!")
                continue
            ...
            if self.amp:
                with autocast('cuda'):
                    ...
                    # Immediately sanity-check model logits for numerical issues.
                    if 'logits' in outputs:
                        logits = outputs['logits']
                        if torch.isnan(logits).any() or torch.isinf(logits).any():
                            print(f"[ERROR] Batch {i}: Model output logits contain NaN/Inf!")
                            print(f"        logits stats: min={logits.min()}, max={logits.max()}, nan_count={torch.isnan(logits).sum()}, inf_count={torch.isinf(logits).sum()}")
                            print(f"        This may indicate model parameters are corrupted. Skipping batch.")
                            continue
                    
                    loss, loss_dict = self.criterion(outputs, masks)
                    loss = loss / self.accum_steps
                    
                    # Skip unstable batches with NaN/Inf losses instead of crashing.
                ...
                if (i + 1) % self.accum_steps == 0:
                    ...
                    if self.grad_clip > 0:
                        ...
                        # Detect NaN/Inf gradients before computing norms.
                        has_nan_grad = False
                        nan_grad_params = []
                        for name, param in self.model.named_parameters():
                            if param.grad is not None:
                                ...
                                if nan_count > 0 or inf_count > 0:
                                    print(f"[ERROR] Batch {i}: Parameter {name} has NaN/Inf gradient! nan_count={nan_count}, inf_count={inf_count}")
                                    nan_grad_params.append(name)
                                    has_nan_grad = True
                        if has_nan_grad:
                            print(f"[ERROR] Batch {i}: Detected NaN/Inf gradients in parameters: {nan_grad_params}")
                            print(f"        Skipping optimizer step")
                            self.optimizer.zero_grad()
                            self.scaler.update()
                            continue
                        
                        # Skip optimizer step only if gradient norm is NaN/Inf.
                        ...
                        if total_norm > 50.0:
                            print(f"[INFO] Large gradient norm: {total_norm:.2f} (clipped to {self.grad_clip})")
                ...
                if torch.isnan(images).any() or torch.isinf(images).any():
                    print(f"[ERROR] Batch {i}: Input images contain NaN/Inf!")
                    continue
                if torch.isnan(masks).any() or torch.isinf(masks.float()).any():
                    print(f"[ERROR] Batch {i}: Input masks contain NaN/Inf!")
                    continue
                ...
                # Sanity-check logits without AMP.
                if 'logits' in outputs:
                    logits = outputs['logits']
                    if torch.isnan(logits).any() or torch.isinf(logits).any():
                        print(f"[ERROR] Batch {i}: Model output logits contain NaN/Inf!")
                ...
                if (i + 1) % self.accum_steps == 0:
                    if self.grad_clip > 0:
                        # Detect NaN/Inf gradients before clipping.
                        has_nan_grad = False
                        nan_grad_params = []
                        for name, param in self.model.named_parameters():
                            if param.grad is not None:
                        ...
                        total_norm = nn.utils.clip_grad_norm_(self.model.parameters(), self.grad_clip)
                        # Skip only if the gradient norm itself is NaN/Inf.
                        if torch.isnan(total_norm) or torch.isinf(total_norm):
                            print(f"[ERROR] Batch {i}: Gradient norm is NaN/Inf: {total_norm}, skipping optimizer step")
                            self.optimizer.zero_grad()
                            continue
            ...
            # Compute hard predictions for metric computation
            ...
            # TensorBoard logging every `log_every` steps.
        ...
        # Handle remaining partial accumulation at the end of the epoch.
    ...
    def validate(self, val_loader):
        """Run validation loop and compute region / boundary metrics."""
        self.model.eval()
        ...
        # If open-set is enabled and OV head is used, but no threshold is set yet,
        # collect a small buffer of OV pixel logits for automatic calibration.
        ...
        for batch in pbar:
            ...
            # Forward pass with optional AMP to speed up validation.
            if self.amp:
                with autocast('cuda'):
                    outputs = self.model(images)
                    loss, loss_dict = self.criterion(outputs, masks)
            ...
            # Collect OV logits for a few mini-batches to calibrate the open-set threshold.
            ...
            # Log one batch of validation image/mask/prediction triplets.
        ...
        # If required, perform a one-shot open-set threshold calibration based on OV logits.
        if calib_buf is not None and len(calib_buf) > 0:
            with torch.amp.autocast('cuda', enabled=self.amp):
                calib_batch = torch.cat(calib_buf, dim=0)  # merge over a few mini-batches
    ...
    def save_checkpoint(self, epoch, scores, is_best=False):
        """Save model checkpoint (last and best according to chosen metric)."""
        checkpoint = {
            'epoch': epoch,
    ...
    def fit(self, train_loader, val_loader, start_epoch=1):
        """Main training loop over all epochs with validation and checkpointing."""
        print(f"Starting training for {self.epochs} epochs")
        print(f"Output directory: {self.output_dir}")
        print(f"Starting from epoch: {start_epoch}")
        
        # Curriculum learning schedule for boundary loss weight
        bd_warmup_epochs = self.config['train'].get('bd_warmup_epochs', 0)
        
        for epoch in range(start_epoch, self.epochs + 1):
            # Linearly ramp boundary loss weight up to its target value.
            ...
            if epoch % self.config['train']['val_freq'] == 0:
                ...
                # Track and save best model according to configured key metric.
                ...
                if self.performance_driven_unfreeze and not self.gate_unfrozen:
                    ...
                    # Unfreeze the gate if validation performance stagnates.
                ...
                # Log aggregated metrics and learning rates to TensorBoard and CSV.
                if self.logger is not None:
